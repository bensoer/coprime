...
fun printHelp(){
    ...
    println("\t\tPRIVKEYCRACK\t\t Attempts to crack and generate the private key using the publishable components of the\n\t\t\t\t\t " +
...
fun crackPrivateKeyOfComponents(e:Long, n:Long){
    println("Private Key Generation From Public Key Components Will Now Begin")
    println("Note this function will not terminate until it has determined all possible private keys have been found. " +
            "It is likely it will find the correct one much sooner then it determines there are no other possible options")
    println("Each result will be printed as they are found along with a benchmark time of how long it took to find the value")
    ...
    while (true){
        ...
        if(isPrimNumber(coPrimePair.first) && isPrimNumber(coPrimePair.second) && (coPrimePair.first * coPrimePair.second) == n){
            ...
            println("This Calculation Process From Start to Component took $differenceMilliSeconds milliseconds OR $differenceSeconds seconds")
        }
    }
}
